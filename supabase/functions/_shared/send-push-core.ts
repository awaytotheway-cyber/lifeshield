/**
 * Shared push send logic used by send-push and other Edge Functions.
 * Never throws — callers must not break their main flow if push fails.
 */
...
export async function sendPushForUser(
  ...
  try {
    ...
    if (tokens.length === 0) {
      // No device registered — not an error; patient may be on Expo Go or denied permission.
      return result;
    }
